...
void printAllPossibleCombination(int n, int row)
{
    ...
    //here we are not printing anything, just place at correct position
    for (int i = 0; i < n; i++)
    {
        if (isPossible(n, row, i))
        {
            ...
            //if we have explored all paths keeping 1 in that cell
            //now backtrack and keep 0 there and next pr 1 rakho and then explore paths
            board[row][i] = 0;
        }
    }
}
